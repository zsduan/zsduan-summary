            ...
            try {
                fetch(settings.pptxFileUrl)
                    .then(response => {
                        ...
                        return response.blob(); // 读取响应为 Blob  
                    })
                    .then(blob => {
                        // 从 URL 中提取文件名  
                        var file_name = settings.pptxFileUrl.split('/').pop();
                        var fileExtension = file_name.split('.').pop();
                        var fileNameWithoutExtension = file_name.replace('.' + fileExtension, '');

                        // 如果需要设置 blob 的 name 属性（注意：Blob 本身没有 name 属性，这里仅为了后续使用）  
                        // blob.name 并不是 Blob 的一部分，这里仅作为示例  
                        var blobWithName = new Blob([blob], { type: 'application/vnd.openxmlformats-officedocument.presentationml.presentation' });
                        blobWithName.name = fileNameWithoutExtension; // 注意：这实际上是不标准的，因为 Blob 没有 name 属性  
                        // 使用 FileReader 读取 Blob  
                        var reader = new FileReader();
                        reader.onload = function (e) {
                            // 这里的 e.target.result 是 ArrayBuffer  
