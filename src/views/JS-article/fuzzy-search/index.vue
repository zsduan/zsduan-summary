<template>
    <section>
        ...
        <public-item title="数组对象关键字模糊搜索" :code="code2">
            <el-input v-model="searchValueByObject" placeholder="请输入搜索内容" @input="onSearchByObject">
                <template slot="prepend">
                    <el-select class="my-select" @change="onSearchByObject" v-model="selectValueByObject"
                        placeholder="请选择搜索字段">
                        <el-option v-for="item in seachType" :key="item.value" :label="item.label" :value="item.value">
                        </el-option>
                    </el-select>
                </template>
            </el-input>
        ...
        <public-item title="数组对象关键字模糊搜索(不带key)" :code="code3">
            <el-input v-model="searchValueByNoKey" placeholder="请输入搜索内容" @input="onSearchByNoKey">

            </el-input>
            {{ searchResultByNoKey }}
        </public-item>
        <public-item title="多层级数组对象关键字模糊搜索" :code="code3" tips="也可以不携带 key">
            <el-input v-model="searchValueByChild" placeholder="请输入搜索内容" @input="onSearchByChild">
                <template slot="prepend">
                    <el-select class="my-select" @change="onSearchByChild" v-model="selectValueByChild"
                        placeholder="请选择搜索字段">
                        <el-option v-for="item in seachType" :key="item.value" :label="item.label" :value="item.value">
                        </el-option>
                    </el-select>
...
export default {
    ...
    methods: {
        ...
        onSearchByNoKey() {
            Search({
                data: this.listByObject,
                value: this.searchValueByNoKey,
                success: (res) => {
                    this.searchResultByNoKey = res;
                }
            })
        }
    }
