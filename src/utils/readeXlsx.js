 ...
 * @author zs.duan
 * @email zs.duan@qq.com
 * @version 1.2.0
 * @last-modify 2024-01-22 16:00:00
 * */
...
export const readeExcel = async (option) => {
    ...
    if (!(option instanceof File) && !option.file) {
        options.fail({ code: -1, msg: "option.file 必填" });
        return
    }
    ...
    let file = options.Vue.$refs[options.file].files[0] || document.querySelector(`#${options.file}`).files[0] || options.file;
    ...
    if (suffix != '.xls' && suffix != '.xlsx') {
        options.fail({ code: -1, msg: "请上传xls和xlsx文件" });
        return;
    }
