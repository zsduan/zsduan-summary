 ...
 * @LastEditors: zs.duan
 * @LastEditTime: 2022-09-30 17:05:32
...
function getTraditionalFestival(mouth ,day){
    // 没有处理腊月二十九过年的问题
    let Festival = traditional_festival.filter(item => {return item.mouth == mouth && item.day == day});
    return Festival.length ?  Festival[0].name : "";
}

// 获取公立节日
function getFestival(mouth ,day){
    let Festival = festival.filter(item => {return item.mouth == mouth && item.day == day});
    return Festival.length ? Festival[0].name : "";
}
...
function getCalendar(time){
    ...
    calendar.lunarFestival = getTraditionalFestival(calendar.lunarMonthCn , calendar.lunarDayCn);
